...
function Find-MmprojForModel {
    ...
    if (-not (Test-ModelSupportsVision -ModelName $null -ModelPath $ModelPath -Architecture $Architecture)) {
        return $null
    }
...
$Models = @(
    foreach ($File in $ModelFiles) {
        ...
        $EffectiveMmprojPath = if ($ExistingEntryIsManual -and $ExistingEntry.PSObject.Properties["mmproj_path"]) {
            [string]$ExistingEntry.mmproj_path
        }
        else {
            $MmprojPath
        }
        if (-not [string]::IsNullOrWhiteSpace($EffectiveMmprojPath)) {
            $Entry["mmproj_path"] = $EffectiveMmprojPath
        }

        $Entry["capabilities"] = if ($ExistingEntryIsManual -and $ExistingEntry.PSObject.Properties["capabilities"]) {
            $ExistingEntry.capabilities
        }
        else {
            Get-ModelCapabilities -ModelName $BaseName -ModelPath $File.FullName
        }
        if ($ExistingEntry) {
        ...
        $Entry["notes"] = if ($ExistingEntryIsManual -and $ExistingEntry.PSObject.Properties["notes"] -and -not [string]::IsNullOrWhiteSpace([string]$ExistingEntry.notes)) {
            [string]$ExistingEntry.notes
        }
        else {
            "Auto-generated by scan_model.ps1."
        }
        $Entry
    }
)
